...
$(document).ready(function() {
	...
	$("#uploadfile").change(function(){
		...
		$('#jqxLoader').jqxLoader('open');
	  	...
	  	$.ajax({
	    	type:"POST",
	    	url: "/PSMES/PSMES_UI210_excel_upload.do",
	    	processData: false,
	    	contentType: false,
	    	data: formData,
	    	success: function(data){
	    		...
	    		$("#grid").jqxGrid('endupdate');
 	      		...
 	      		//console.log("## PSMES_UI210_excel_upload data: ", data)
	...
	$('#selectCompanyBtn').click(function () {
		//alert("업체 선택");
		//업체 선택 popup
		//showPopup("/PSMES/PSMES_UI211.do", "selectCompanyPopup", "900", "600", "yes", "", "", "yes", "yes", "yes");
	...
	$('#btnAttch').click(function () {
		//alert("업체 선택");
		//업체 선택 popup
		showPopup("/PSMES/PSMES_UI214.do?pROrderPk=" + $("#pROrderPk").val(), "selectAttchFilePopup", "910", "600", "yes", "", "", "yes", "yes", "yes");
	});
    ...
    $('#btnfileUp').click(function (e) {
       // e.preventDefault();
		...
		showPopup("/PSMES/PSMES_UI213.do?pROrderPk="+$('#pROrderPk').val(), "dwgFilePopup", "900", "600", "yes", "", "", "yes", "yes", "yes");
    });
	...
	$('#btn1').click(function () {
		...
		for(var i=0;i<rowindexes.length;i++)
		{
			...
			if(isEmpty($("#grid").jqxGrid('getcellvalue', i, "R_ORDER_DETAIL_PK")))
			{
				rpkCnt++;
			}
		}  
		...
		{
			window.parent.addTabNewLoad('/PSMES/PSMES_UI310.do?programId=PSMES_UI210', '발주 등록 및 조회', JSON.stringify(rows), value1, value2, value3,'','');
		}
	...
	$('#btn2').click(function () {
		...
		if(value1 != null && value1 !="")
		{
			window.parent.addTabNewLoad('/PSMES/PSMES_UI410.do?programId=PSMES_UI210', '작업지시', JSON.stringify(rows), value1, value2, value3, value4,value5);
		}
		else
		{
	...
	$('#grid').on('cellclick', function (event) {
	    ...
	    if(dataField == "DRAWING_FILE")
	    {  
	    	if($("#grid").jqxGrid('getcellvalue', rowBoundIndex, "DRAWING_FILE") != "" ){
		    	location.href = "/PSMES/PSMES_UI210_FileDown.do?filePath="+$('#pROrderPk').val()+"&fileName="+$("#grid").jqxGrid('getcellvalue', rowBoundIndex, "DRAWING_FILE");
	    	}
	    }
	});
	...
	$('#grid').on('cellvaluechanged', function (event) {
	    ...
	    // row's bound index.
	    var rowBoundIndex = args.rowindex;
	    if(args.datafield == "UNIT_COST" || args.datafield == "AMOUNT")
		{
			var unitCost = nvl($('#grid').jqxGrid('getcellvalue', rowBoundIndex, "UNIT_COST"),0);
			var amount = nvl($('#grid').jqxGrid('getcellvalue', rowBoundIndex, "AMOUNT"),0);

			$('#grid').jqxGrid('setcellvalue', rowBoundIndex, "SUM_COST", unitCost*amount );
		}else if(args.datafield == "OUT_YN"){
			if($('#grid').jqxGrid('getcellvalue', rowBoundIndex, "OUT_YN") == "Y"){
				$('#grid').jqxGrid('setcellvalue', rowBoundIndex, "SELF_YN", "N" );
			}else{
				$('#grid').jqxGrid('setcellvalue', rowBoundIndex, "SELF_YN", "Y" );
			}
		}else if(args.datafield == "SELF_YN"){
			if($('#grid').jqxGrid('getcellvalue', rowBoundIndex, "SELF_YN") == "Y"){
				$('#grid').jqxGrid('setcellvalue', rowBoundIndex, "OUT_YN", "N" );
			}else{
				$('#grid').jqxGrid('setcellvalue', rowBoundIndex, "OUT_YN", "Y" );
			}
		}
	});
...
//그리드 콤보관련 시작                    
var outYn = [{CD_V:"Y",CD_NM:"Y"},{CD_V:"N",CD_NM:"N"}];
...
var selfYn = [{CD_V:"Y",CD_NM:"Y"},{CD_V:"N",CD_NM:"N"}];
...
var setColumn = [
	...
	{ text: '품번'	, columntype: 'textbox'		, datafield: 'PRODUCT_NO'	, cellsalign: 'center', align: 'center',  width: 100, exportable: true },
	{ text: '품명'	, columntype: 'textbox'		, datafield: 'PRODUCT_NAME'	, cellsalign: 'left', align: 'center',  width: 200, exportable: true },
	{ text: '규격'	, columntype: 'textbox'		, datafield: 'R_STANDARD'	, cellsalign: 'left', align: 'center',  width: 200, exportable: true },
	{ text: '단가'	, columntype: 'numberinput'	, datafield: 'UNIT_COST'	, cellsalign: 'right', align: 'center',  width: 150, cellsformat: 'D', exportable: true,
		createeditor: function (row, cellvalue, editor) {
            editor.jqxNumberInput({min: 0, decimalDigits: 0, digits: 10 });
        }	 },
	{ text: '수량'	, columntype: 'numberinput'	, datafield: 'AMOUNT'	, cellsalign: 'center', align: 'center',  width: 80, cellsformat: 'D', exportable: true,
		createeditor: function (row, cellvalue, editor) {
            editor.jqxNumberInput({min: 0, decimalDigits: 0, digits: 10 });
        }	 },
	{ text: '금액'	, columntype: 'numberinput'	, datafield: 'SUM_COST'	, cellsalign: 'right', align: 'center',  width: 150, cellsformat: 'D',editable: false , exportable: true},
	{ text: '외주'	, columntype: 'textbox'	, datafield: 'OUT_YN'	, cellsalign: 'center', align: 'center',  width: 200, editable: false, exportable: false },
...
//        }
// 	}, 
	{ text: '자체제작'	, columntype: 'combobox'	, datafield: 'SELF_YN', displayfield: 'SELF_YN'		, cellsalign: 'center', align: 'center', width: 100, exportable: false, hidden: true,
		createeditor: function (row, value, editor) {
            editor.jqxComboBox({ source: selfYnAdapter, displayMember: 'CD_NM', valueMember: 'CD_V', promptText: "선택"});
       }
	}, 
	{ text: '도면'	, columntype: 'textbox'		, datafield: 'DRAWING_FILE'	, cellsalign: 'center', align: 'center',  width: 200, editable:false,  cellsrenderer: cellsrenderer, exportable: false },
	{ text: '비고'	, columntype: 'textbox'		, datafield: 'NOTE'	, cellsalign: 'left', align: 'center', exportable: true },
	{ text: 'R_ORDER_DETAIL_PK'	, columntype: 'textbox', datafield: 'R_ORDER_DETAIL_PK'	, hidden: true, exportable: false },
...
var fn_Main_save = function () {
  ...
  //로딩이미지 오픈
	...
	$.ajax({
      url: "/PSMES/PSMES_UI210_Main_save.do",
      dataType: 'json',
...
var fn_del = function()
{
	...
	$.ajax({
    url: "/PSMES/PSMES_UI210_delete.do",
    dataType: 'json',
    data : $("#form1").serialize(),
		//data : param,
    type : 'POST',
...
var fn_delRow = function(grid_id)
{
  ...
  //로딩이미지 오픈
	...
	$.ajax({
      url: "/PSMES/PSMES_UI210_delRow.do",
      dataType: 'json',
      data : $("#form1").serialize(),
		//data : param,
      type : 'POST',
...
function fn_main_search () {
	...
	pUrl = "/PSMES/PSMES_UI210_searchMas.do";
	...
	$.ajax({
        ...
        success: function (data) {
            // initailize grid
        	$("#pROrderPk").val(data.MastInfo.R_ORDER_PK);
        	$("#pROrderNo").val(data.MastInfo.R_ORDER_NO);
        	$("#pROrderName").val(data.MastInfo.R_ORDER_NAME);
        	$("#pCompanyCode").val(data.MastInfo.COMPANY_CODE);
        	$("#pCompanyName").val(data.MastInfo.COMPANY_NAME);
        	$("#pROrderDate").val(data.MastInfo.R_ORDER_DATE);
        	$("#pFixedDate").val(data.MastInfo.FIXED_DATE);
        	$("#pShippingDate").val(data.MastInfo.SHIPPING_DATE);
        	$("#pEstimatedCost").val(data.MastInfo.ESTIMATED_COST);
        	$("#pGeneralCost").val(data.MastInfo.GENERAL_COST);
        	$("#pFinalCost").val(data.MastInfo.FINAL_COST);
        	$("#pRemarks").val(data.MastInfo.REMARKS);
        	$("#pNationTp").val(data.MastInfo.NATION_TP);
        	$("#pSalesTp").val(data.MastInfo.SALES_TP);
        	$("#pOutsourcingCost").val(data.MastInfo.OUTSOURCING_COST);
...
function fn_detail_search () {
	...
	pUrl = "/PSMES/PSMES_UI210_searchDet.do";
										...
										<tr>
											...
											<td>
												<select class="form-control"  name = "pNationTp" id = "pNationTp">
													...
													<option value="${nationTp.CD_V}">${nationTp.CD_NM}</option>
													</c:forEach>
												</select>
											</td>
										</tr>
										...
										<tr>
											...
											<td>
												<select class="form-control"  name = "pSalesTp" id = "pSalesTp">
													...
													<option value="${salesTp.CD_V}">${salesTp.CD_NM}</option>
													</c:forEach>
												</select>
											</td>
											<th>특이사항</th>
