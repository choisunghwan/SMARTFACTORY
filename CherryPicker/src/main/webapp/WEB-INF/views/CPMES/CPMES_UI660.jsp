...
// 	$('#mainSplitter').jqxSplitter({ width: 'calc(100% - 2px)', height: 'calc(100% - 170px)', orientation: 'horizontal', panels: [{ size: 500 }] });
	...
	$('#btnSearch').click(function () {
		fn_search_n("/CPMES/CPMES_UI660_searchList.do", "grid",setColumn,"100%");
...
function setData3(data)
{
	...
	var data1 = new Array();
	 ...
	 else
	 {
		 for(var i=0;i<12;i++)
		 {
			 data1.push(data.response[i].SUM_FINAL_COST_MONTH_CP);
			 data2.push(data.response[i].SUM_GENERAL_COST_MONTH_CP);
			 data3.push(data.response[i].P_COST_MONTH_CP);
		 }	
	 }	 
