...
//			        }
				  ...
				  {
					  System.out.println("여기오니?? " + page);
					  
					  response.setContentType("text/html; charset=UTF-8");
					  
					  PrintWriter out = response.getWriter();
					   
					  out.println("<script>alert('세션이 만료되었습니다. 로그인화면으로 이동합니다.'); window.parent.location.href='/login.do';window.parent.parent.location.href='/login.do'</script>");
...
//    		user = (UserVO)session.getAttribute("UserInfo");
//    		
//    		logger.info("유저 정보는 있나=== > "+ user.getsUserId());
