...
// 		}	
	...
	<c:forEach items="${WorkDayList}" var="WorkDayList">
		...
		rdata.CD_V = "${WorkDayList.CD_V}";
		rdata.ATTR1 = "${WorkDayList.ATTR1}";
		rdata.ATTR2 = "${WorkDayList.ATTR2}";
		rdata.ATTR3 = "${WorkDayList.ATTR3}";
		rdata.ATTR4 = "${WorkDayList.ATTR4}";
	...
	if(!isEmpty($("#programId").val()) !="" && $("#programId").val() == "PSMES_UI210")
	{
		...
		for(i=0;i<comWorkDayArray.length;i++){
			...
			if( intervalDay <= parseInt(comWorkDayArray[i].CD_V) ){
				newObj = comWorkDayArray[i]
				break;
			}
		}
		
		var workDay1 = dt1.getDateSumDay(parseInt(newObj.ATTR1));
		var workDay2 = dt1.getDateSumDay(parseInt(newObj.ATTR2));
		var workDay3 = dt1.getDateSumDay(parseInt(newObj.ATTR3));
		var workDay4 = dt1.getDateSumDay(parseInt(newObj.ATTR4));
		...
		for(var i=0;i< prows.length;i++)
		{
			...
			rowsdata.PRODUCT_NO =  prows[i].PRODUCT_NO;
			rowsdata.PRODUCT_NAME =  prows[i].PRODUCT_NAME;
			rowsdata.R_STANDARD =  prows[i].R_STANDARD;
			rowsdata.AMOUNT =  prows[i].AMOUNT;
			rowsdata.R_ORDER_PK =  param1;
			rowsdata.R_ORDER_DETAIL_PK =  prows[i].R_ORDER_DETAIL_PK;
			//오늘 날짜
			rowsdata.P001_DATE = workDay1;
			rowsdata.P002_DATE = workDay2;
			rowsdata.P003_DATE = workDay3;
			rowsdata.P004_DATE = workDay4;
    ...
    $("#ROrderLov").change(function(){
        ...
        $('#param1').val("");
		fn_search("/PSMES/PSMES_UI410_search.do", "grid",setColumn,"100%");
    });
...
var setColumn = [
	...
	{ text: '품명'	 , columntype: 'textbox', datafield: 'PRODUCT_NAME'	, cellsalign: 'left'  , align: 'center', editable: false},
	{ text: '규격'	 , columntype: 'textbox', datafield: 'R_STANDARD'	, cellsalign: 'left'  , align: 'center', width: 300, editable: false},
	{ text: '수량'	 , columntype: 'textbox', datafield: 'AMOUNT'		, cellsalign: 'center', align: 'center', width: 70 , editable: false},
	{ 
		text: '제관', datafield: 'P001_DATE', columntype: 'datetimeinput',  align: 'center', cellsalign: 'center', width: 150, columngroup: 'ProductDetails1', cellsformat: 'yyyy-MM-dd',
...
// 		}
	...
	{ 
		text: '용접', datafield: 'P002_DATE', columntype: 'datetimeinput',  align: 'center', cellsalign: 'center', width: 150, columngroup: 'ProductDetails1', cellsformat: 'yyyy-MM-dd',
// 	  	validation: function (cell, value) {
// 			if (value == "")
// 				return true; 
// 			var year = value.getFullYear();
...
// 		}
	...
	{ 
		text: '가공', datafield: 'P003_DATE', columntype: 'datetimeinput',  align: 'center', cellsalign: 'center', width: 150, columngroup: 'ProductDetails1', cellsformat: 'yyyy-MM-dd',
// 	  	validation: function (cell, value) {
// 			if (value == "")
// 				return true; 
// 			var year = value.getFullYear();
...
// 		}
	...
	{ 
		text: '조립', datafield: 'P004_DATE', columntype: 'datetimeinput',  align: 'center', cellsalign: 'center', width: 150, columngroup: 'ProductDetails1', cellsformat: 'yyyy-MM-dd',
// 	  	validation: function (cell, value) {
// 			if (value == "")
// 				return true; 
// 			var year = value.getFullYear();
...
var fn_save = function () {
  //저장 전 체크 
	...
	for(var i=0;i<rowindexes.length;i++)
	{
		...
		if($("#grid").jqxGrid('getcellvalue', rowindexes[i], "PRODUCT_NO") == "")
		{
			productNo++;	
		}
		if($("#grid").jqxGrid('getcellvalue', rowindexes[i], "PRODUCT_NAME") == "")
		{
			productNm++;	
		}
							...
							<div class="form-row">
								...
								<div class="col">
									...
									<select class="form-control"  name = "ROrderLov" id = "ROrderLov" style="width:300px;">
										...
										<option value="${ROrderLov.CD_V}">${ROrderLov.CD_NM}</option>
										</c:forEach>
									</select>
								</div>
							</div>
