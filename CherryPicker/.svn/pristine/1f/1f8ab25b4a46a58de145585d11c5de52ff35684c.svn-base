...
$(document).ready(function() {
	...
	$('#fa-calendar3').click(function () {
		var url = '/PSMES/PSMES_UI511.do';
		
		showPopup(url, 'PSMES_UI511', 1240, 700, /*center*/ 1, '', '', /*status*/ 0, /*scroll*/ 0, /*resize*/ 0, /*menubar*/ 0, /*toolbar*/ 0, /*locat*/ 0, /*fullscreen*/ 0);
	...
	$("#grid").on('cellendedit', function (event) 
	{
	    ...
	    if(dataField == "UNIT_COST")
	    {
	    	unitCost = args.value;
	    	if(!isEmpty($("#grid").jqxGrid('getcellvalue', rowBoundIndex, "AMOUNT")))
	    	{
	    		amount = $("#grid").jqxGrid('getcellvalue', rowBoundIndex, "AMOUNT");
	    	}
	    	
	    	$("#grid").jqxGrid('setcellvalue', rowBoundIndex, "SUM_COST", Number(unitCost) * Number(amount));
	    	
	    	$("#grid").jqxGrid('endcelledit', rowBoundIndex, "SUM_COST", true);
	    ...
	    {  
	    	...
	    	if(!isEmpty($("#grid").jqxGrid('getcellvalue', rowBoundIndex, "UNIT_COST")))
	    	{
	    		unitCost = $("#grid").jqxGrid('getcellvalue', rowBoundIndex, "UNIT_COST");  
	    	}
	...
	/*발주목록에서 넘어오면 조회*/
	if($("#programId").val() !="" && ($("#programId").val() == "PSMES_UI320" || $("#programId").val() == "PSMES_UI330"))
	{
		var param1 = $('#param1', parent.document).val();	// 수주PK
		//var param1 = $('#param1').val();
		$('#P_ORDER_PK').val(param1);
	...
	if(!isEmpty($("#programId").val()) !="" && $("#programId").val() == "PSMES_UI210")
	{
		$('#grid').jqxGrid('clearselection');
		...
		for(var i=0;i< prows.length;i++)
		{
			...
			{  
				...
				rowsdata.P_PRODUCT_NO =  prows[i].PRODUCT_NO;
				rowsdata.P_PRODUCT_NAME =  prows[i].PRODUCT_NAME;
				rowsdata.P_STANDARD =  prows[i].R_STANDARD;
				rowsdata.UNIT_COST =  prows[i].UNIT_COST;
				rowsdata.AMOUNT =  prows[i].AMOUNT;
				rowsdata.SUM_COST =  prows[i].SUM_COST;
				rowsdata.R_ORDER_DETAIL_PK =  prows[i].R_ORDER_DETAIL_PK;
				//rowsdata.R_ORDER_DETAIL_PK =  prows[i].R_ORDER_DETAIL_PK;
...
// 		var param3 = $('#param3').val();
		...
		$('#R_ORDER_PK').val(param1);
		$('#R_ORDER_NO').val(param2);
		$('#R_ORDER_NAME').val(param3);
...
// 	$('#FINAL_COST').keyup(function () {

// 		var finalcost = $(this).val();

// 		$("#FINAL_COST").val(addComma(finalcost));
// 	});
...
function fn_searchMas() {
	...
	url = "/PSMES/PSMES_UI310_searchMas.do";
...
//         	xmlHttpRequest.setRequestHeader("AJAX", "true");
        ...
        success: function (data) {
            ...
            $("#P_ORDER_NO").val(data.response[0].P_ORDER_NO);
            $("#P_ORDER_PK").val(data.response[0].P_ORDER_PK);
            $("#P_ORDER_NAME").val(data.response[0].P_ORDER_NAME);
            $("#COMPANY_CODE_NM").val(data.response[0].COMPANY_CODE_NM);
            $("#COMPANY_CODE").val(data.response[0].COMPANY_CODE);
            $("#P_ORDER_DATE").val(data.response[0].P_ORDER_DATE);
            $("#FIXED_DATE").val(data.response[0].FIXED_DATE);
            $("#STOCK_DATE").val(data.response[0].STOCK_DATE);
            $("#STAND_YN").val(data.response[0].STAND_YN);
            $("#ESTIMATED_COST").val(data.response[0].ESTIMATED_COST);
            $("#FINAL_COST").val(data.response[0].FINAL_COST);
            $("#R_ORDER_NO").val(data.response[0].R_ORDER_NO);
            $("#FINAL_COST").val(data.response[0].FINAL_COST);
            $("#R_ORDER_NAME").val(data.response[0].R_ORDER_NAME);
            $("#REMARKS").val(data.response[0].REMARKS);
            
            $('#jqxLoader').jqxLoader('close');
            
            $("#ESTIMATED_COST").val(addComma($("#ESTIMATED_COST").val()));
            $("#FINAL_COST").val(addComma($("#FINAL_COST").val()));
            
            $("#R_ORDER_NO").val(data.response[0].R_ORDER_NO);
            $("#R_ORDER_NAME").val(data.response[0].R_ORDER_NAME);
            
            
            fn_search("/PSMES/PSMES_UI310_searchDet.do", "grid",setColumn,"100%");
...
var fn_estimateSet = function()
{
	var rows = $('#grid').jqxGrid('getrows');
 	...
 	{ 
	    ...
	    {  
 			if(!isEmpty($("#grid").jqxGrid('getcellvalue', i, "SUM_COST")))
 			{
 				sumCost += $("#grid").jqxGrid('getcellvalue', i, "SUM_COST");	  
 			}
 			   
	    }	   
 	
 		$("#ESTIMATED_COST").val(addComma(sumCost));
 	}
}
...
var setColumn = [
	...
	{ text: '품번'		, columntype: 'textbox'		, datafield: 'P_PRODUCT_NO'		, cellsalign: 'center', align: 'center',  width: '100' },
	{ text: '품명'		, columntype: 'textbox'		, datafield: 'P_PRODUCT_NAME'	, cellsalign: 'left', align: 'center',  width: '200' },
	{ text: '규격'		, columntype: 'textbox'		, datafield: 'P_STANDARD'		, cellsalign: 'left', align: 'center',  width: '200' },
	{ text: '단가'		, columntype: 'numberinput'		, datafield: 'UNIT_COST'	, cellsalign: 'right' , align: 'center',  width: '150', cellsformat: 'D'
		, createeditor: function (row, cellvalue, editor) {
            editor.jqxNumberInput({min: 0, decimalDigits: 0, digits: 10 });
        }	
	},
	{ text: '수량'		, columntype: 'numberinput'		, datafield: 'AMOUNT'	, cellsalign: 'right' , align: 'center',  width: '150', cellsformat: 'D'
		, createeditor: function (row, cellvalue, editor) {
            editor.jqxNumberInput({min: 0, decimalDigits: 0, digits: 10 });
        }	
	},
	{ text: '공급가액'		, columntype: 'numberinput'		, datafield: 'SUM_COST'	, cellsalign: 'right' , align: 'center',  width: '150', cellsformat: 'D', editable:false
		, createeditor: function (row, cellvalue, editor) {
            editor.jqxNumberInput({min: 0, decimalDigits: 0, digits: 10 });
        }	
	},

	{ text: '비고'		, columntype: 'textbox'		, datafield: 'NOTE'				, cellsalign: 'left', align: 'center' },
	{ text: '발주PK'		, columntype: 'textbox'		, datafield: 'P_ORDER_PK'	, hidden: true},
	{ text: '발주상세PK'	, columntype: 'textbox'		, datafield: 'P_ORDER_DETAIL_PK', hidden: true},
	{ text: '수주상세PK'	, columntype: 'textbox'		, datafield: 'R_ORDER_DETAIL_PK', hidden: true},
...
var generaterow = function (i) {    
    ...
    row["SUM_COST"] = "0";
    row["NOTE"] = "";
    row["P_ORDER_DETAIL_PK"] = "";
    row["R_ORDER_DETAIL_PK"] = ""; 
...
var fn_del = function(grid_id)
{
	...
	$.ajax({
      url: "/PSMES/PSMES_UI310_delete.do",
      dataType: 'json',
      data : $("#form1").serialize(),
		//data : param,
      type : 'POST',
...
var fn_rowdel = function(grid_id)
{
	...
	for(var i=0;i<rowindexes.length;i++)
	{
		...
		if($('#grid').jqxGrid('getcellvalue', rowindexes[i], "P_ORDER_DETAIL_PK") == "")
		{
			delrows[i] = rowindexes[i];
			nonedelCnt++;
		}
		else if($('#grid').jqxGrid('getcellvalue', rowindexes[i], "P_ORDER_DETAIL_PK") != "")
		{
			delCnt++;  
		}	
	}
	...
	else if(delCnt >0) 
	{
		...
		$.ajax({
	      url: "/PSMES/PSMES_UI310_row_del.do",
	      dataType: 'json',
	      data : $("#form1").serialize(),
			//data : param,
	      type : 'POST',
...
function returnValue(arr){
	$("#COMPANY_CODE").val(arr[0]);
	$("#COMPANY_CODE_NM").val(arr[1]);
} 
...
function fn_reportView(){
	...
	// PO_NO Check
	$.ajax({
		url : '/PSMES/PSMES_UI310_Report.do',
		type : 'post',
		data : {P_ORDER_PK : $("#P_ORDER_PK").val()},
		dataType : 'json',
...
<input type="hidden" name="ORDER_TP" id="ORDER_TP" value="">
<input type="hidden" name="R_ORDER_PK" id="R_ORDER_PK" value="">
								...
								<table class="type-a" id="table">
									...
									<tbody>
										<tr>
											<th>발주번호</th>
											<td>
												<input type="text" style="width:100%;" class="form-control" id="P_ORDER_NO" name="P_ORDER_NO" value="">
												<input type="hidden" id="P_ORDER_PK" name="P_ORDER_PK" value="">
											</td> 
											<th>발주명</th>
											<td colspan="3">
												<input type="text" style="width:100%;" class="form-control" id="P_ORDER_NAME" name="P_ORDER_NAME">
											</td>
											<th>업체명</th>  
											<td>
												<div class="col">	
													<input type="text" name="COMPANY_CODE_NM" id="COMPANY_CODE_NM"  value="" class="form-control" style="width:100%;" >													
													<input type="hidden" name="COMPANY_CODE" id="COMPANY_CODE"  value="" class="form-control" style="width:120px;" >
										...
										<tr>
											...
											<td>
												<div class="col">	
													<input type="text" name="P_ORDER_DATE" id="P_ORDER_DATE"  value="<%=pSearchEndDt %>" class="form-control" style="width:120px;" >
													<button type="button" class="date_ico" id ="fa-calendar" >달력</button>
												</div>
											</td>
											<th>납기일</th>
											<td>
												<div class="col">	
													<input type="text" name="FIXED_DATE" id="FIXED_DATE"  value="" class="form-control" style="width:120px;" >
													<button type="button" class="date_ico" id ="fa-calendar1" >달력</button>
												</div>
											</td>
											<th>입고일</th>
											<td>
												<div class="col">	
													<input type="text" name="STOCK_DATE" id="STOCK_DATE"  value="" class="form-control" style="width:120px;" >
													<button type="button" class="date_ico" id ="fa-calendar2" >달력</button>
												</div>
											</td>
											<th>일반구매</th>
											<td>
												<select class="form-control"  name = "STAND_YN" id = "STAND_YN" style="width:100%;">
										...
										<tr>
											<th>견적가</th>
											<td>
												<input type="text" style="width:100%;" class="form-control" id="ESTIMATED_COST" name="ESTIMATED_COST" readonly>
											</td>
											<th>결정가</th>
											<td>
												<input type="text" style="width:100%;" class="form-control" id="FINAL_COST" name="FINAL_COST">
											</td>
											<th>수주번호</th>
											<td>
												<input type="text" style="width:100%;" class="form-control" id="R_ORDER_NO" name="R_ORDER_NO">
												
											</td>
											<th>수주명</th>
											<td>
												<input type="text" style="width:100%;" class="form-control" id="R_ORDER_NAME" name="R_ORDER_NAME">
											</td>
										</tr>
										<tr>
											<th>특이사항</th>
											<td colspan="7">
												<input type="text" style="width:100%;" class="form-control" id="REMARKS" name="REMARKS">
											</td>
										</tr>
									</tbody>
								</table>
