package com.CherryPicker.web.ui400;
...
import com.CherryPicker.web.ui400.UI400Service;
...
public class UI400Controller {
	...
	// 작업지시
	//////////////////////////////////////////////////
	@RequestMapping(value="/CPMES/CPMES_UI410.do")
    public ModelAndView CPMES_UI410(CommandMap commandMap) throws Exception{
    	ModelAndView mv = new ModelAndView("/CPMES/CPMES_UI410");
    	
    	commandMap.put("R_ORDER_PK_CP", (String)commandMap.get("param1"));
		List<Map<String,Object>> list = UI400Service.searchROrderLov(commandMap.getMap());
		mv.addObject("ROrderLov", list);
		
		List<Map<String,Object>> list1 = UI400Service.CPMES_UI410_searchWorkDateList();
		mv.addObject("WorkDayList", list1);
		
    	return mv;
    }
	
	@RequestMapping(value="/CPMES/CPMES_UI410_search.do")
    public ModelAndView CPMES_UI410_search(CommandMap commandMap, HttpServletRequest request) throws Exception{
		
		List<Map<String,Object>> list = UI400Service.CPMES_UI410_search(commandMap.getMap());
	...
	@RequestMapping(value="/CPMES/CPMES_UI410_save.do")
    public ModelAndView CPMES_UI410_save(CommandMap commandMap) throws Exception{
		...
		for(int i=0; i<iCnt; i++){
			...
			if("1".equals(chk))
			{
				...
				// 수주테이블 작업지시 상태 변경
				if(i==0) {
					UI400Service.CPMES_UI410_save_status(smap);
				}
				
				smap.put("R_ORDER_DETAIL_PK_CP", CommonUtils.isNull(jsonObj.get("R_ORDER_DETAIL_PK_CP")));
				smap.put("P1_PLAN_DATE_CP", CommonUtils.isNull(jsonObj.get("P001_DATE_CP")));
				smap.put("P2_PLAN_DATE_CP", CommonUtils.isNull(jsonObj.get("P002_DATE_CP")));
				smap.put("P3_PLAN_DATE_CP", CommonUtils.isNull(jsonObj.get("P003_DATE_CP")));
				smap.put("P4_PLAN_DATE_CP", CommonUtils.isNull(jsonObj.get("P004_DATE_CP")));
		...
		UI400Service.CPMES_UI410_save(list);
	...
	// 생산실적조회
	//////////////////////////////////////////////////
	@RequestMapping(value="/CPMES/CPMES_UI420.do")
    public ModelAndView CPMES_UI420(CommandMap commandMap) throws Exception{
    	ModelAndView mv = new ModelAndView("/CPMES/CPMES_UI420");
    	
    	return mv;
    }
	
	@RequestMapping(value="/CPMES/CPMES_UI420_main_search.do")
    public ModelAndView CPMES_UI420_main_search(CommandMap commandMap, HttpServletRequest request) throws Exception{
		
		List<Map<String,Object>> list = UI400Service.CPMES_UI420_main_search(commandMap.getMap());
	...
	@RequestMapping(value="/CPMES/CPMES_UI420_detail_search.do")
    public ModelAndView CPMES_UI420_detail_search(CommandMap commandMap, HttpServletRequest request) throws Exception{
		
		List<Map<String,Object>> list = UI400Service.CPMES_UI420_detail_search(commandMap.getMap());
	...
	@RequestMapping(value="/CPMES/CPMES_UI420_save.do")
    public ModelAndView CPMES_UI420_save(CommandMap commandMap, HttpServletRequest request) throws Exception{
		...
		for(int i=0; i<iCnt; i++){
			...
			if("1".equals(chk))
			{
				R_ORDER_PK = CommonUtils.isNull(jsonObj.get("R_ORDER_PK_CP"));
				smap.put("R_ORDER_PK_CP", CommonUtils.isNull(jsonObj.get("R_ORDER_PK_CP")));
				smap.put("UPDATE_USER_CP", sUserId);
		...
		if(list.size() > 0) {
			UI400Service.CPMES_UI420_save(list);			
		}
		
		
		// 작업완료시 상세공정 작업완료 일괄처리
		Map<String, Object> smap1 = new HashMap<String, Object>();
		smap1.put("R_ORDER_PK_CP", R_ORDER_PK);
		smap1.put("UPDATE_USER_CP", sUserId);
		UI400Service.CPMES_UI420_WORK_LIST_save(smap1);
